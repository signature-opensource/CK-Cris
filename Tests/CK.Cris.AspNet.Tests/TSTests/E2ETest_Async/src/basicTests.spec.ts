...
import { HttpCrisEndpoint, AmbientValues, CrisError, UserMessageLevel, SimpleUserMessage, AspNetCrisResultError } from "@local/ck-gen"; 
...
it('A command can return a SimpleMessage.', async () => {
  const ep = new HttpCrisEndpoint(axios, crisEndpoint);
  const cmd = new CommandWithMessage();
